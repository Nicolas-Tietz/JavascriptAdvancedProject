...
async function newsCheck(){
    try{
        ...
        for (let test of sortedNews){
            newsContainer.insertAdjacentElement("afterbegin",test);
...
async function addNews(news){
    ...
    if (response.data == null){
        ...
        console.log('La news è ',div);
        notSortedNews.push(div);
        console.log('Pushata una news in notsortednews',notSortedNews);
...
function updateNewsTime(){
    for (let news of newsHtmlElemsArray){
        //Dalla news prendo il primo elemento span
        let spanTime = (news.getElementsByTagName('span')).item(0);
        console.log(news,'spantime',spanTime);
        let time = spanTime.innerHTML;
        
        time = parseInt(time);
        time+=minutesPassed;
        time = (timeNumber(time));
        let timeStr = timeString(time);
        spanTime.innerHTML = time;
        
        spanTime.nextElementSibling.innerHTML = ' '+timeStr;
